...
@bp_time.route("/time-tracker/timesheet/week-lock", methods=["GET"])
def get_timesheet_week_lock():
    """Return lock status for an employee's week based on submitted timesheet entries."""
    try:
        employee_id = (request.args.get("employee_id") or "").strip()
        ...
        if not employee_id or not week_start or not week_end:
            return jsonify({"success": False, "error": "employee_id, week_start and week_end are required"}), 400

        try:
            start_dt = datetime.fromisoformat(week_start)
            end_dt = datetime.fromisoformat(week_end)
        except ValueError:
            return jsonify({"success": False, "error": "Invalid ISO date format for week_start or week_end"}), 400

        if end_dt < start_dt:
            return jsonify({"success": False, "error": "week_end cannot be before week_start"}), 400

        entries = _read_ts_entries()
        relevant = []
        target_emp = employee_id.upper()
        for rec in entries:
            rec_emp = str(rec.get("employee_id") or "").strip().upper()
            if rec_emp != target_emp:
                continue
        ...
        pending = sum(1 for r in relevant if str(r.get("status") or "").lower() == "pending")
        accepted = sum(1 for r in relevant if str(r.get("status") or "").lower() == "accepted")
        rejected = sum(1 for r in relevant if str(r.get("status") or "").lower() == "rejected")
